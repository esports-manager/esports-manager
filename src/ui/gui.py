...
class GUI:
    ...
    # def app(self) -> None:
    #     v = [0]
    #     teams = self._get_all_team_objects()
    #     while True:
    #         event, values = self.window.read()
    #         if event in [sg.WINDOW_CLOSED, 'exit_main']:
    #             break
    #         elif event == 'new_game':
    #             self.window['main_screen'].update(visible=False)
    #             self.window['create_manager'].update(visible=True)
    #         elif event == 'load_game_main':
    #             self.window['main_screen'].update(visible=False)
    #             self.window['load_game'].update(visible=True)
    #         elif event == 'cancel_load':
    #             self.window['load_game'].update(visible=False)
    #             self.window['main_screen'].update(visible=True)
    #         elif event == 'cancel_new_game':
    #             self.window['create_manager'].update(visible=False)
    #             self.window['main_screen'].update(visible=True)
    #         elif event == 'team_list':
    #             value = values['team_list']
    #             v = self.window.Element('team_list').Values[value[0]]
    #             self.window.Element('player_list').Update(values=get_player_names(v, teams))
    #
    #         print(event, values)
    #         print(v[0])
    #
    ...
    def create_manager_layout(self) -> list:
        """
    ...
    def load_game_layout(self) -> list:
        """
    ...
    # debugging
    def debug_window(self, match) -> sg.Window:
        icon_path = self._encode_icon()

        create_look_and_feel()
        sg.theme('EsmTheme')

        layout1 = sg.Column(self.get_debug_layout(match), visible=False, element_justification='center')
        layout2 = sg.Column(self.picks_and_bans_layout(match), visible=True, element_justification='center')
    ...
    def get_team_data(self, match: MatchLive = None):
    ...
    def get_debug_layout(self, match: MatchLive = None):
    ...
    def picks_and_bans_layout(self, match: MatchLive = None):
        players = [[player for player in team.list_players] for team in match.match.teams]
