...
def test_get_inbox_empty(client: TestClient):
    resp = client.get("/api/moba/inbox")
    assert resp.status_code == 200
    assert resp.json() == []


def test_create_inbox_message(client: TestClient, session: Session):
    resp = client.post(
        "/api/moba/inbox",
        json={
            "subject": "Welcome",
    ...
    msg = session.get(MobaInbox, inbox_id)
...
def test_get_inbox_message_by_id(client: TestClient, session: Session):
    m = MobaInbox(subject="Test", body="Body", category=MobaInboxCategory.MARKET)
    session.add(m)
    session.commit()
    session.refresh(m)
    resp = client.get(f"/api/moba/inbox/{m.id}")
    assert resp.status_code == 200
    body = resp.json()
    assert body["id"] == m.id
...
def test_get_inbox_message_not_found(client: TestClient):
    resp = client.get("/api/moba/inbox/9999")
    assert resp.status_code == 404
    assert resp.json() == {"detail": "Inbox message not found"}


def test_mark_read_via_get_by_id(client: TestClient, session: Session):
    m = MobaInbox(subject="Read me", body="...", category=MobaInboxCategory.GENERAL)
    session.add(m)
    session.commit()
    session.refresh(m)
    assert m.status == MobaInboxStatus.UNREAD
    resp = client.get(f"/api/moba/inbox/{m.id}?mark_read=true")
    assert resp.status_code == 200
...
def test_update_inbox_message(client: TestClient, session: Session):
...
def test_delete_inbox_message(client: TestClient, session: Session):
    m = MobaInbox(subject="Bye", body="Body", category=MobaInboxCategory.ADMIN)
    session.add(m)
    session.commit()
    session.refresh(m)
    resp = client.delete(f"/api/moba/inbox/{m.id}")
    assert resp.status_code == 200
