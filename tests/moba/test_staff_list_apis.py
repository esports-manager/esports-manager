...
async def test_staff_list_filter_by_team(client: AsyncClient, session: AsyncSession):
    ...
    resp = await client.get("/api/moba/staff", params={"team_id": t1.id})
    assert resp.status_code == 200
    body = resp.json()
    assert [it["id"] for it in body] == [s1.id]


async def test_staff_list_search(client: AsyncClient, session: AsyncSession):
    ...
    # first_name
    r1 = await client.get("/api/moba/staff", params={"search": "ali"})
    assert r1.status_code == 200
    assert [i["first_name"] for i in r1.json()] == ["Alice"]

    # last_name
    r2 = await client.get("/api/moba/staff", params={"search": "son"})
    assert r2.status_code == 200
    assert [i["last_name"] for i in r2.json()] == ["Johnson"]

    # nickname
    r3 = await client.get("/api/moba/staff", params={"search": "az"})
...
async def test_staff_list_invalid_role_is_ignored(
    ...
    resp = await client.get("/api/moba/staff", params={"role": "does_not_exist"})
    assert resp.status_code == 200
